...
LogStream &LogStream::operator<<(int val) {
    ...
    char *buf = buffer_.current();
    if (sizeof(int) == 2) {
        auto len = Util::i16toa(static_cast<int16_t>(val), buf);
        buffer_.add(len);
...
LogStream &LogStream::operator<<(unsigned int val) {
    ...
    char *buf = buffer_.current();
    if (sizeof(int) == 2) {
        auto len = Util::u16toa(static_cast<uint16_t>(val), buf);
        buffer_.add(len);
    } else if (sizeof(int) == 4) {
        auto len = Util::u32toa(val, buf);
        buffer_.add(len);
...
LogStream &LogStream::operator<<(long val) {
    ...
    if (sizeof(long) == 2) {
        ...
        auto len = Util::i32toa(static_cast<int32_t>(val), buf);
        buffer_.add(len);
    } else {
        auto len = Util::i64toa(static_cast<int64_t>(val), buf);
        buffer_.add(len);
...
LogStream &LogStream::operator<<(unsigned long val) {
    ...
    if (sizeof(long) == 2) {
        ...
        auto len = Util::u32toa(static_cast<uint32_t>(val), buf);
        buffer_.add(len);
    } else {
        auto len = Util::u64toa(static_cast<uint64_t>(val), buf);
        buffer_.add(len);
